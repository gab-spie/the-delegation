            ...
            <div className="space-y-6">
              ...
              {renderField('LLM Model', <Cpu size={12} />, isView ? (
                <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-100 border border-zinc-200 rounded-lg text-xs font-mono text-zinc-600 w-fit lowercase">
                  {editData.model || 'claude-sonnet-4-6'}
                </div>
              ) : (
                <select
                  value={editData.model || 'claude-sonnet-4-6'}
                ...
                >
                  <optgroup label="Claude (Anthropic)">
                    {availableModels.filter((m: string) => m.startsWith('claude-')).map((m: string) => <option key={m} value={m}>{m}</option>)}
                  </optgroup>
                  <optgroup label="Gemini (Google)">
                    {availableModels.filter((m: string) => !m.startsWith('claude-')).map((m: string) => <option key={m} value={m}>{m}</option>)}
                  </optgroup>
                </select>
              ), 'LLM model this agent will use for reasoning.')}
            </div>

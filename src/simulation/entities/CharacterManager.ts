...
import { getAllAgents, getAllCharacters, MAX_AGENTS } from '../../data/agents';
...
export class CharacterManager {
  private instanceCount = MAX_AGENTS + 1; // Pre-allocate for max agents to avoid resize issues
  ...
  public setInstanceCount(count: number) {
    // Never shrink below the pre-allocated buffer — only grow when needed.
    // This prevents Safari WebGPU from losing instances when the default team
    // (fewer agents) triggers a reinit after MAX_AGENTS+1 was already allocated.
    if (count <= this.instanceCount) return;
  ...
  private initInstances() {
    ...
    for (let i = 0; i < this.instanceCount; i++) {
      const agentNode = allCharacters.find(a => a.index === i);

      // Slot has no real agent in the current team → sink underground so it's invisible.
      // This handles pre-allocated buffer slots beyond the active team size.
      if (!agentNode) {
        posArray[i * 4 + 0] = 0;
        posArray[i * 4 + 1] = -1000; // underground / invisible
        posArray[i * 4 + 2] = 0;
        posArray[i * 4 + 3] = 0;
        colorArray[i * 3 + 0] = 0;
        colorArray[i * 3 + 1] = 0;

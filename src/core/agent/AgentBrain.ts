...
import { isClaudeModel } from '../llm/constants';
...
export class AgentBrain {
  ...
  public async think(prompt: string, options: ThinkOptions = {}): Promise<{ text: string, toolCalls: any[] }> {
    ...
    try {
      ...
      const provider = isClaudeModel(model)
        ? (() => { if (!llmConfig.claudeApiKey) throw new Error('Claude API key is required. Add it via the key icon.'); return new ClaudeProvider(llmConfig.claudeApiKey); })()
        : (() => { if (!llmConfig.geminiApiKey) throw new Error('Gemini API key is required. Add it via the key icon.'); return new GeminiProvider(llmConfig.geminiApiKey); })();
  ...
  public async processFinalAsset(prompt: string, options: any) {
    ...
    try {
      const llmConfig = useUiStore.getState().llmConfig;
      if (!llmConfig.geminiApiKey) throw new Error('Gemini API key is required for media generation. Add it via the key icon.');

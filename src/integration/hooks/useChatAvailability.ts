...
export function useChatAvailability(agentIndex: number | null): ChatAvailability {
  ...
  // Allow lead agent to keep chatting after project is done (for follow-up instructions)
  if (phase === 'done') return isLead ? { canChat: true, reason: '' } : { canChat: false, reason: 'Project completed' }

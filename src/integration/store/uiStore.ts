    ...
    llmConfig: (() => {
      try {
        ...
        if (saved) {
          ...
          // Migrate old single apiKey → claudeApiKey
          if (parsed.apiKey && !parsed.claudeApiKey && !parsed.geminiApiKey) {
            parsed.claudeApiKey = '';
      ...
      return { claudeApiKey: '', geminiApiKey: '', model: DEFAULT_MODELS.text };
    })(),
